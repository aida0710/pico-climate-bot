...
PALETTE = bytes((245, 247, 250, 255, 255, 255, 23, 40, 59,
                 221, 229, 237, 188, 88, 30, 23, 111, 155, 82, 100, 122))
...
def _write_png(canvas, path, feed):
    ...
    with open(path, 'wb') as output:
        ...
        for y in range(HEIGHT):
            feed()
            header = struct.pack('<BHH', 1 if y == HEIGHT - 1 else 0, size, size ^ 65535)
            output.write(header)
...
def _draw(fb, history, now, feed):
    ...
    fb.text('7 DAYS / END %02d-%02d %02d:%02d JST' % (date[1], date[2], date[3], date[4]), 12, 21, 6)
    left, right = 48, 464
    ...
    for record in history.iter_records(now):
        ...
        for index in range(2):
            ...
            fb.fill_rect(max(left, x - 1), max(top, y - 1), min(3, right - max(left, x - 1) + 1), min(3, bottom - max(top, y - 1) + 1), color)
