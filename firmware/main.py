...
def collect_message():
    ...
    try:
        ...
        delta = (
            ...
            else "{:+.2f}hPa".format(pressure - prev_pressure)
        )
        prev_pressure = pressure
        lines.append(
            "🌡 BMP180温度: {:.1f}°C  🌬 気圧: {:.2f}hPa  (変化: {})".format(
...
def send_report(message, connected=None):
    ...
    try:
        ...
        if now < 1704067200:
            ...
            return False
        history.append(
            now, last_measurement.get("temperature"), last_measurement.get("humidity")
        )
...
def main():
    ...
    while True:
        try:
            ...
            if (
                connected
                and not clock_synced
                and (not skip_ntp or dashboard is not None)
            ):
                sync_clock()
            skip_ntp = False
            ...
            sleep_safe(
                max(INTERVAL_S, dashboard.retry_after if dashboard is not None else 0)
            )
        except Exception as exc:
