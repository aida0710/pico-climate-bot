...
class Dashboard:
    def __init__(self, token, channel_id, bot_id, state_path='discord_state.json', http=None, feed=lambda: None):
        ...
        try:
            ...
            if (not isinstance(self.state, dict) or self.state.get('channel') != self.channel_id
            ...
            raise RuntimeError('Discord state is unreadable; refusing duplicate message')
    ...
    def find_existing(self):
        self.feed()
        response = self.http.request('GET', API+'/channels/'+self.channel_id+'/messages?limit=10',
        ...
        for message in messages:
            if message.get('author', {}).get('id') != self.bot_id:
                continue
            if any(embed.get('footer', {}).get('text') == MARKER for embed in message.get('embeds', [])):
                if found is not None:
                    raise RuntimeError('Multiple dashboard messages; explicit selection required')
                found = message['id']
        self.feed()
        return found
    ...
    def update(self, message, png_path):
        ...
        if not message_id and self.state.get('pending'):
            raise RuntimeError('Previous message creation unresolved; refusing duplicate')
        nonce = binascii.hexlify(os.urandom(8)).decode()
        payload = {'content':'', 'allowed_mentions':{'parse':[]},
                   'embeds':[{'title':'お家の温度・湿度',
                              'description':message+'\n\n直近7日間の推移（未取得期間は空白）',
                              'color':0x168C9C, 'footer':{'text':MARKER},
                              'image':{'url':'attachment://climate.png'}}],
                   'attachments':[{'id':0,'filename':'climate.png',
        ...
        headers['Content-Length'] = str(len(prefix)+os.stat(png_path)[6]+len(suffix))
        ...
        try:
            response = self.http.request(method, url, headers=headers,
                data=self.chunks(prefix, png_path, suffix), timeout=5)
            status = response.status_code
            print('BOT_HTTP', status, method)
            if status == 429:
                try:
                    self.retry_after = max(1, int(read_json(response, feed=self.feed).get('retry_after', 300))+1)
                except Exception:
