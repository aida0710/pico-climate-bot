...
def firmware_files(root):
    ...
    if (root / 'config.py').exists():
        raise ValueError('firmware/config.py must not contain secrets; config stays on the device')
    result = {}
...
def backup(device, names, root):
    ...
    private_write(dest / 'manifest.json', json.dumps(
        {'format': 1, 'managed': names, 'files': records}, indent=2).encode())
    return dest
...
def deploy(device, files, backup_root):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError('Deployment and rollback failed. Recover from ' + str(snapshot)) from None
        raise failure
