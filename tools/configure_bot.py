...
def updated_config(source, settings):
    ...
    for node in ast.parse(text).body:
        if isinstance(node, ast.Assign) and any(
            isinstance(t, ast.Name) and t.id in FIELDS for t in node.targets
        ):
    ...
    for key, value in zip(
        FIELDS,
        (settings["bot_token"], settings["channel_id"], settings["application_id"]),
    ):
...
def main():
    ...
    if not all(settings.get(k) for k in ("bot_token", "channel_id", "application_id")):
        raise ValueError(
            "Bot credential file needs bot_token, channel_id, application_id"
        )
