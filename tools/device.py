...
def select_port(explicit=None):
    ...
    if len(available) != 1:
        raise ValueError(
            "Specify --port; expected one USB device, found %d" % len(available)
        )
    return available[0]
...
class Device:
    def __init__(self, port):
        ...
        try:
            board = (
                self.raw.execute("import os\nprint(os.uname().machine)")
                .decode()
                .strip()
            )
            if board != "Raspberry Pi Pico W with RP2040":
                raise ValueError(
                    "Unsupported board (expected Raspberry Pi Pico W with RP2040)"
                )
            self.raw.execute(
    ...
    def stage(self, name, data):
        ...
        try:
            for offset in range(0, len(data), 512):
                self.execute(
                    "_upload.write(ubinascii.unhexlify(%r))"
                    % data[offset : offset + 512].hex()
                )
        finally:
