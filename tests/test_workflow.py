...
class FakeDevice:
    def __init__(self):
        self.files = {'main.py': b'old main', 'libs/bmp180.py': b'old driver', 'config.py': b'PRIVATE'}
        self.failed = False
    ...
    def remove(self, name): self.files.pop(name, None)

class WorkflowTests(unittest.TestCase):
    def setUp(self):
        module_path = ROOT / 'tools/workflow.py'
        self.assertTrue(module_path.exists(), 'USB deployment workflow is not implemented')
    ...
    def test_backup_is_private_and_records_hashes(self):
        d = FakeDevice();p = self.w.backup(d, ['main.py','libs/bmp180.py'], self.root)
        ...
        self.assertEqual(self.w.load_backup(p), {'main.py':b'old main','libs/bmp180.py':b'old driver'})
    def test_deploy_keeps_config_and_replaces_main_last(self):
        d=FakeDevice();self.w.deploy(d, {'main.py':b'new main','libs/bmp180.py':b'new driver'}, self.root)
    ...
    def test_install_failure_rolls_back_all_files(self):
        d=FakeDevice();original=dict(d.files);d.fail_target='main.py'
        with self.assertRaises(OSError):
            self.w.deploy(d, {'main.py':b'new main','libs/bmp180.py':b'new driver'}, self.root)
    ...
    def test_staging_failure_leaves_original_files(self):
        ...
        d=StagingFailure();original=dict(d.files)
        with self.assertRaises(OSError):
            self.w.deploy(d,{'main.py':b'new main','libs/bmp180.py':b'new driver'},self.root)
        self.assertEqual(d.files,original)
    def test_rollback_removes_new_files(self):
        d=FakeDevice();original=dict(d.files);d.fail_target='main.py'
        with self.assertRaises(OSError):
            self.w.deploy(d,{'extra.py':b'new file','main.py':b'new main'},self.root)
        self.assertEqual(d.files,original)
    def test_unchanged_firmware_does_not_write_flash(self):
        d=FakeDevice();self.w.deploy(d, {'main.py':b'old main','libs/bmp180.py':b'old driver'},self.root)
        self.assertEqual(d.writes,[])
    def test_modified_backup_is_rejected(self):
        d=FakeDevice();p=self.w.backup(d,['main.py'],self.root)
        (p/'main.py').write_bytes(b'tampered')
        with self.assertRaises(ValueError):self.w.load_backup(p)
    ...
    def test_firmware_config_is_rejected(self):
        f=self.root/'firmware';f.mkdir();(f/'main.py').write_text('pass\n');(f/'config.py').write_text('password="secret"')
        with self.assertRaises(ValueError):self.w.firmware_files(f)
    def test_unsafe_backup_path_is_rejected(self):
        import json
        (self.root/'manifest.json').write_text(json.dumps({'managed':['../oops.py'],'files':{}}))
        with self.assertRaises(ValueError):self.w.load_backup(self.root)
