...
class Response:
    def __init__(self,status=200,result=None):self.status_code=status;self.result=result;self.closed=False;self.raw=io.BytesIO(json.dumps(result).encode())
    def json(self):return self.result
    def close(self):self.closed=True

class HTTP:
    def __init__(self):self.calls=[];self.messages=[];self.lost=False;self.status=None;self.responses=[]
    def request(self,method,url,**kwargs):
        data=kwargs.get('data');body=b''.join(data) if data is not None and not isinstance(data,bytes) else data
        self.calls.append((method,url,kwargs,body))
        if method=='GET':r=Response(result=self.messages)
        elif self.status:r=Response(self.status,{'retry_after':8})
        else:
            r=Response(result={'id':'444'})
            if method=='POST':
                self.messages=[{'id':'444','author':{'id':'111'},'embeds':[{'footer':{'text':'Pico W | 7-day climate'}}]}]
                if self.lost:raise OSError('response lost')
...
class PicoDiscordTests(unittest.TestCase):
    def setUp(self):
        ...
        m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m);self.m=m
        self.tmp=tempfile.TemporaryDirectory();self.addCleanup(self.tmp.cleanup)
        self.path=Path(self.tmp.name);self.png=self.path/'climate.png';self.png.write_bytes(b'PNG'*3000)
        self.http=HTTP();self.feeds=[]
        self.client=self.new_client()
    def new_client(self):
        return self.m.Dashboard('secret','123','111',state_path=str(self.path/'state.json'),http=self.http,feed=lambda:self.feeds.append(1))
    ...
    def test_stream_content_length_and_replacement(self):
        ...
        self.assertIn(b'"attachments"',body);self.assertIn(b'"files[0]"',body)
        self.assertIn(b'"allowed_mentions"',body)
    def test_response_loss_reconciles_without_duplicate(self):
        self.http.lost=True
        with self.assertRaises(OSError):self.client.update('reading',str(self.png))
        self.assertTrue(self.new_client().update('reading2',str(self.png)))
    ...
    def test_pending_unknown_stops_new_posts(self):
        ...
        with self.assertRaises(OSError):self.client.update('reading',str(self.png))
        self.http.messages=[]
        with self.assertRaises(RuntimeError):self.new_client().update('reading',str(self.png))
        self.assertEqual(sum(c[0]=='POST' for c in self.http.calls),1)
    def test_oversized_response_is_bounded(self):
        ...
        with self.assertRaises(ValueError):self.m.read_json(r)
    ...
    def test_deleted_message_does_not_post_again(self):
        self.client.update('reading',str(self.png));self.http.status=404
        self.assertFalse(self.client.update('reading',str(self.png)))
        self.assertEqual(sum(c[0]=='POST' for c in self.http.calls),1)
