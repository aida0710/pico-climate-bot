...
class FakeFrameBuffer:
    ...
    def line(self, x0, y0, x1, y1, color):
        ...
        for i in range(steps + 1):
            self.pixel(round(x0+(x1-x0)*i/steps), round(y0+(y1-y0)*i/steps), color)
...
def load_chart():
    spec = importlib.util.spec_from_file_location('pico_chart_test_target', Path(__file__).parents[1] / 'firmware/pico_chart.py')
    module = importlib.util.module_from_spec(spec)
    with patch.dict(sys.modules, framebuf=types.SimpleNamespace(FrameBuffer=FakeFrameBuffer, GS4_HMSB=2)):
...
class PicoChartTests(unittest.TestCase):
    ...
    def draw(self, rows):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = self.chart.render(History(rows), self.NOW, path, lambda: calls.append(1))
            self.assertEqual(result, path)
            data = Path(path).read_bytes()
        chunks, raw = decode_png(data)
        self.assertEqual(struct.unpack('>IIBBBBB', chunks[b'IHDR']), (480, 272, 4, 3, 0, 0, 0))
    ...
    def test_single_point_visible_and_missing_data_not_connected(self):
        ...
        self.assertTrue(any(fb.pixel(x, y) == 4 for y in range(272) for x in range(480)))
    ...
    def test_gap_and_missing_value_break_lines(self):
        fb, _ = self.draw([(self.NOW-3000, 20, 40), (self.NOW-2700, None, 41), (self.NOW-2400, 22, 42), (self.NOW-60, 24, 44)])
    ...
    def test_png_preserves_high_nibble_first_pixels(self):
        ...
        for y in (0, 32, 100, 271):
            self.assertEqual(raw[y*241+1:(y+1)*241], bytes(fb.buffer[y*240:(y+1)*240]))
