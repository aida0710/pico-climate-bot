...
class Clock:
    ...
    def ticks_ms(self):
        ...
        if self.calls > 200000:
            raise TimeoutError("test execution budget")
        self.now += 1
...
class Response:
    ...
    @property
    def text(self):
        raise AssertionError("Do not read unbounded error body")
...
def application():
    ...
    ns = {
        ...
        "config": types.SimpleNamespace(
            WIFI_SSID="test",
            WIFI_PASSWORD="test",
            DISCORD_WEBHOOK_URL="https://invalid.test/",
        ),
        "json": __import__("json"),
        "gc": types.SimpleNamespace(collect=lambda: None),
        "os": types.SimpleNamespace(
            stat=os.stat, remove=os.remove, rename=os.rename, sync=lambda: None
        ),
        "VERSION": "test",
        "INTERVAL_S": 300,
...
class ReliabilityTests(unittest.TestCase):
    ...
    def test_bmp_temperature_and_pressure_waits_survive_wrap(self):
        cls = next(
            n
            for n in ast.parse((ROOT / "libs/bmp180.py").read_text()).body
            if isinstance(n, ast.ClassDef)
        )
        fn = next(
            n
            for n in cls.body
            if isinstance(n, ast.FunctionDef) and n.name == "makegauge"
        )
        for start in (1000, PERIOD - 3, PERIOD - 20):
            ...
            bus = types.SimpleNamespace(
                writeto_mem=lambda *a: None, readfrom_mem=lambda a, r, n: bytes(n)
            )
            sensor = types.SimpleNamespace(
                _bmp_i2c=bus, _bmp_addr=119, oversample_setting=3
            )
            g = ns["makegauge"](sensor)
            self.assertTrue(
                any(next(g) is True for _ in range(100)),
                f"wait did not complete at {start}",
            )
    ...
    def test_bmp_stuck_conversion_has_deadline(self):
        ...
        fn = next(
            n
            for n in cls.body
            if isinstance(n, ast.FunctionDef) and n.name == "blocking_read"
        )
    ...
    def test_wifi_failure_is_bounded_across_wrap(self):
        ns = application()
        ns["wlan"] = WLAN(False)
        ns["time"] = Clock(PERIOD - 10000)
        try:
            result = ns["connect_wifi"]()
        except TimeoutError:
            self.fail("Wi-Fi wait has no deadline")
        self.assertFalse(result)
        self.assertLess(ns["time"].calls, 1000)

    def test_send_reconnects_and_has_timeout(self):
        ns = application()
        r = Response()
        ns["urequests"] = Requests(r)
    ...
    def test_offline_does_not_enter_dns_http(self):
        ...
        ns["urequests"] = Requests(Response())
        try:
            result = ns["send_to_discord"]("test")
        except TimeoutError:
            self.fail("offline send never returned")
        self.assertFalse(result)
        self.assertEqual(ns["urequests"].calls, [])
    ...
    def test_transport_exception_returns_failure_without_secrets_in_log(self):
        ...
        def fail(*a, **kw):
            raise OSError("secret-webhook-value")

        ns["urequests"] = types.SimpleNamespace(post=fail)
        self.assertFalse(ns["send_to_discord"]("test"))
        for p in pathlib.Path(".").glob("*.log*"):
            self.assertNotIn("secret-webhook-value", p.read_text())

    def test_five_minute_sleep_feeds_watchdog(self):
        ns = application()
        self.assertIn("sleep_safe", ns)
        ns["sleep_safe"](300)
        self.assertGreaterEqual(ns["wdt"].feeds, 300)
        self.assertGreaterEqual(ns["time"].now, 300000)

    def test_ntp_failure_is_nonfatal(self):
        ns = application()
        self.assertIn("sync_clock", ns)
    ...
    def test_log_is_rate_limited_and_rotated(self):
        ...
        self.assertLessEqual(
            sum(p.stat().st_size for p in pathlib.Path(".").glob("health.log*")), 8192
        )
    ...
    def test_sensor_failure_does_not_stop_other_measurements(self):
        ns = application()
        self.assertIn("collect_message", ns)
        ...
        self.assertIn("DHT11", result)
        self.assertIn("BMP180", result)
        self.assertIsNone(ns["bmp"])
