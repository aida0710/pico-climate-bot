...
class RingHistoryTests(unittest.TestCase):
    ...
    def test_wrap_reboot_and_next_append_preserve_order_and_bound(self):
        ...
        self.assertEqual(
            [r[0] for r in h.iter_records(BASE + 13 * 300)],
    ...
    def test_window_includes_boundary_and_excludes_future(self):
        ...
        self.assertEqual(
            [r[0] for r in h.iter_records(BASE + WEEK)], [BASE, BASE + WEEK]
        )
    ...
    def test_crc_damage_and_torn_tail_recovered(self):
        broken = bytearray(record(BASE + 1))
        broken[4] ^= 1
        pathlib.Path(self.path).write_bytes(
            record(BASE) + broken + record(BASE + 2)[:7]
        )
        h = self.history()
        ...
        h = self.history()
        self.assertEqual(
            list(h.iter_records(BASE + 3)), [(BASE, 20.0, 50.0), (BASE + 3, 21.0, 51.0)]
        )

    def test_wrapped_newest_corruption_recovery(self):
        h = self.history()
        for i in range(6):
            h.append(BASE + i, 20, 50)
        with open(self.path, "r+b") as stream:
            stream.seek(12)
            stream.write(b"\xff" * 12)
        h = self.history()
        self.assertEqual(
            [r[0] for r in h.iter_records(BASE + 6)], [BASE + 2, BASE + 3, BASE + 4]
        )
        h.append(BASE + 6, 20, 50)
        self.assertEqual(
            [r[0] for r in self.history().iter_records(BASE + 6)],
            [BASE + 2, BASE + 3, BASE + 4, BASE + 6],
        )

    def test_valid_crc_does_not_accept_invalid_record_data(self):
        pathlib.Path(self.path).write_bytes(
            record(1) + record(BASE, 999, 500) + record(BASE + 1)
        )
        self.assertEqual(
            list(self.history().iter_records(BASE + 1)), [(BASE + 1, 20.0, 50.0)]
        )
    ...
    def test_watchdog_fed_during_scan_and_iteration(self):
        pathlib.Path(self.path).write_bytes(
            b"".join(record(BASE + i) for i in range(130))
        )
        calls = []
